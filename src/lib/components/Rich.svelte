...
<a href="{mark.data.uri}" target="{mark.data.uri.indexOf('http') === 0 ? '_blank' : '_self'}">
  ...
  {#if mark.data.uri.startsWith('https://apps.apple.com')}
    <Apple />
  {:else if mark.data.uri.startsWith('https://play.google.com')}
    <Google />
  {:else}
...
{:else if node.nodeType === 'unordered-list'}
  <ul class="flex flex--gapped flex--center">
    {#each node.content as item}<li class="col col--4of12 col--mobile--12of12">{#each item.content as node}{@render n(node)}{/each}</li>{/each}
  </ul>
